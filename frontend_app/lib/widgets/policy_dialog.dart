...
我们主要通过第三方服务 easyScholar API 获取期刊与文章元数据（例如期刊名称、ISSN、文章标题、作者、DOI、出版日期等）。easyScholar 的数据采集、更新与标准化流程可能与原始出版机构存在差异，因此我们提供的数据在某些细节上可能与原始出版物不同步或存在不完整之处。

使用这些数据时请注意：

  1) 准确性与核验：easyScholar 提供的元数据可能包含延迟或不准确信息。在对法律、学术引用或其他需要精确来源的场景下，应以原始出版物或官方渠道为准并进行核验。

  2) 协议与合规：easyScholar 对其 API 的使用有特定服务条款与授权要求。我们在后端调用该 API 并遵守其条款；你在使用本应用内显示的期刊数据时，也需遵循 easyScholar 的相关协议与限制。若有疑问，请查阅 easyScholar 官方文档或联系我们获取更多信息。

  3) 缓存与更新：为提升性能，我们可能会缓存从 easyScholar 获取的元数据，后台会定期同步更新，但可能存在延迟。我们会尽量在界面或文档中注明数据来源以保持透明性。
...
我们可能会不时更新本隐私政策。继续使用服务即表示你接受更新后的政策。
''';
